/* $Id: xiic_dyn_master.c,v 1.1 2007/04/04 18:13:51 wre Exp $ */
/******************************************************************************
*
...
* sent by using XIic_mDynSend7BitAddress().
*
...
int XIic_DynMasterSend(XIic * InstancePtr, u8 *TxMsgPtr, u8 ByteCount)
{
	 ...
	 * Dynamic mode.
	 */
	XIic_mDynSend7BitAddress(InstancePtr->BaseAddress,
				 InstancePtr->AddrOfSlave,
				 XIIC_WRITE_OPERATION);

	/*
	 * Set the transmit address state to indicate the address has been sent for
	 * communication with event driven processing.
	 */
...
static void DynSendMasterData(XIic * InstancePtr)
{
	 ...
	 */
		...
		if (InstancePtr->Options & XII_REPEATED_START_OPTION) {
			...
			XIic_mDynSendStop(InstancePtr->BaseAddress,
...
*			  BusNotBusy interrupt is enabled which will update the EventStatus
*			  when the bus is no longer busy.
*			- XST_IIC_GENERAL_CALL_ADDRESS indicates the slave address is set
*			  to the general call address. This is not allowed for Master
*			  receive mode.
*
* @note		The receive FIFO threshold is a zero based count such that 1 must
*			be subtracted from the desired count to get the correct value. When
* 			receiving data it is also necessary to not receive the last byte
* 			with the prior bytes because the acknowledge must be setup before
...
int XIic_DynMasterRecv(XIic * InstancePtr, u8 *RxMsgPtr, u8 ByteCount)
{
	 ...
	 * address in the FIFO because the transmitter will immediately start when
	...
	XIo_Out8(InstancePtr->BaseAddress + XIIC_RFD_REG_OFFSET, RxFifoOccy);

	/*
	 * Send the Seven Bit address. Only 7 bit addressing is supported in Dynamic
	 * mode and mark that the address has been sent.
	 */
	XIic_mDynSend7BitAddress(InstancePtr->BaseAddress,
	 ...
	 */
	XIic_mDynSendStop(InstancePtr->BaseAddress, ByteCount);
...
static void DynRecvMasterData(XIic * InstancePtr)
{
	...
	if ((InstancePtr->RecvByteCount - BytesInFifo) == 1) {
		/*
		 * Disable Tx error interrupt to prevent interrupt as this device will
		 * cause it when it set NO ACK next.
		 */
		XIic_mDisableIntr(InstancePtr->BaseAddress,
				  XIIC_INTR_TX_ERROR_MASK);
		...
		/*
		 * Read one byte to clear a place for the last byte to be read which
		 * will set the NO ACK.
		 */
	 ...
	 */
		...
		/*
		 * Read data from the FIFO then set zero based FIFO read depth for a
		 * byte.
		 */
