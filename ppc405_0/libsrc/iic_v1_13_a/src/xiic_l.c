/* $Id: xiic_l.c,v 1.3 2007/05/31 11:27:37 mta Exp $ */
/******************************************************************************
*
...
unsigned XIic_DynRecv(u32 BaseAddress, u8 Address, u8 *BufferPtr, u8 ByteCount)
{
	...
	/*
	 * Send the 7 bit slave address for a read operation and set the state to
	 * indicate the address has been sent. Upon writing the address, a start
	 * condition is initiated. MSMS is automatically set to master when the
	 * address is written to the Fifo. If MSMS was already set, then a re-start
	 * is sent prior to the address.
	 */
	...
	/*
	 * The receive is complete. Return the number of bytes that were received.
	 */
	return ByteCount - RemainingByteCount;
}
...
static unsigned DynRecvData(u32 BaseAddress, u8 *BufferPtr, u8 ByteCount)
{
	...
	while (ByteCount > 0) {
		...
		/*
		 * Read in byte from the Rx Fifo. If the Fifo reached the programmed
		 * occupancy depth as programmed in the Rx occupancy reg, this read
		 * access will un throttle the bus such that the next byte is read from
		 * the IIC bus.
		 */
		*BufferPtr++ = XIo_In8(BaseAddress + XIIC_DRR_REG_OFFSET);
		ByteCount--;
		      ...
		      u8 ByteCount, u8 Option)
{
	...
	/*
	 * Put the address into the Fifo to be sent and indicate that the operation
	 * to be performed on the bus is a write operation. Upon writing the
	 * address, a start condition is initiated. MSMS is automatically set to
	 * master when the address is written to the Fifo. If MSMS was already set,
			    ...
			    u8 ByteCount, u8 Option)
{
	...
	while (ByteCount > 0) {
		/*
		 * Wait for the transmit to be empty before sending any more data by
		...
		while (1) {
			...
			if (IntrStatus & (XIIC_INTR_TX_ERROR_MASK |
					  ...
					  XIIC_INTR_BNB_MASK)) {
				/*
				 * Error condition (NACK or ARB Lost or BNB Error Has occurred.
				 * Clear the Control register to send a STOP condition on the
				 * Bus and return the number of bytes still to transmit.
				 */
		...
		/*
		 * Send data to Tx Fifo. If a stop condition is specified and the
		 * last byte is being sent, then set the dynamic stop bit.
		 */
		if ((ByteCount == 1) && (Option == XIIC_STOP)) {
			/*
			 * The MSMS will be cleared automatically upon setting dynamic
			 * stop.
			 */
...
int XIic_DynInit(u32 BaseAddress)
{
	...
	/*
	 * Set receive Fifo occupancy depth for maximum depth (zero based).
	 */
	XIo_Out8(BaseAddress + XIIC_RFO_REG_OFFSET, IIC_RX_FIFO_DEPTH - 1);

	/*
	 * Set transmit Fifo occupancy depth for maximum depth (zero based).
	 */
	XIo_Out8(BaseAddress + XIIC_TFO_REG_OFFSET, IIC_TX_FIFO_DEPTH - 1);

	/*
	 * Set 10bit address.
	 */
	XIo_Out8(BaseAddress + XIIC_TBA_REG_OFFSET, 0);
